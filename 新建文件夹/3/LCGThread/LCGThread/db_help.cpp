...
class DB_Help {
	...
	public:
		...
		//插入到history表
		int insert_history(const History& ht) {
			...
			//		int i = mysql_num_rows(result);
			//		if (i != 0) {
			//			cout << "have history" << endl;
			//			return 0;
			//		}
			//	}
			//}
			sprintf_s(sql, 200, "insert into history (ip,address,time) values('%s','%s','%s')", ht.ip.c_str(), ht.address.c_str(),ht.time.c_str());
			cout << sql << endl;
			//c_str()将string转为char*
			mysql_query(mysql, "SET NAMES GBK");//设置编码格式
			res = mysql_query(mysql, sql);
